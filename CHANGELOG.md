...
- Trust Transition Lineage v1 with root-private append-only record files, deterministic record hashes, previous-record chaining, full accepted-state snapshots, local baseline/reconciliation controls, Accepted Owner Trust State head anchoring, updater integration, and Trust Inspector verification.
- Installed Release/File Integrity v1 with a strict root-owned accepted-candidate inventory, Git-object-derived SHA-256 file identities, split verification of active `/opt/open-mmi` source runtime and venv `site-packages`, old-trusted-side candidate wheel construction/verification, prepared-update integrity gating, and separate Trust Inspector evidence for local byte integrity versus release-signer provenance.
- Release Provenance / Pinned Signer Root v1 with create-once root-owned OpenPGP trust state, full-fingerprint local owner bootstrap, offline isolated verification of current and candidate commits against fixed root-controlled Git/GPG binaries, updater pre-build provenance gating, no automatic key discovery/rotation, and Trust Inspector provenance evidence kept separate from capability authorization.

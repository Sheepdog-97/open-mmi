...
Trust Inspector v1 is read-only and does not authorize capabilities or change trust state. `PASS` means one concrete local check succeeded; `UNVERIFIED` is not a pass and means the current architecture cannot prove the claim; `FAIL` means observed state contradicts the inspected contract. The built-in inspector is not an independent attestation root: until signed installed-file integrity, accepted owner release state, transition lineage and an independent verifier exist, a modified privileged installation can also modify its own inspector. Treat removal of a previously available inspection check, secret VIN-binding material appearing in inspector output, or a false `PASS` for known-unverified enforcement as a trust-boundary defect.
